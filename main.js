...
async function verificDir(client) {
    ...
    try {
        fs.readdir('disparar/numeros/', (err, files) => {
            if (err)
                console.log(err);
            else {
                // console.clear();
...
async function readFile(client, fileName) {
    ...
    try {
        fs.readFile(`disparar/numeros/${fileName}`, 'utf-8', function (err, data) {
...
async function prepareWhats(client, data, fileName) {
    ...
    try {
        ...
        fs.readFile('disparar/texto/mensagem.txt', 'utf-8', function (err, data) {
            if (err) {
                saveLog(err);
                stateAppRun = false;
                return false;
...
async function moveFileFinally(fileName) {
    ...
    try {
        var lastPath = `disparar/numeros/${fileName}`;
        var newPath = `disparar/enviados/${fileName}`;
